...
RegisterNetEvent('GetTruckerPay', function()
    ...
    if TruckNetID and TruckNetID > 0 and NetworkDoesNetworkIdExist(TruckNetID) then
        truckEntity = NetworkGetEntityFromNetworkId(TruckNetID)
    end

    if TrailerNetID and TrailerNetID > 0 and NetworkDoesNetworkIdExist(TrailerNetID) then
        trailerEntity = NetworkGetEntityFromNetworkId(TrailerNetID)
    end
    ...
    if stationsRefueled > 0 then
        ...
        stationsRefueled = 0
        RemoveBlip(blip)
        if truckEntity and truckEntity ~= 0 and DoesEntityExist(truckEntity) then
            DeleteVehicle(truckEntity)
        end
        if trailerEntity and trailerEntity ~= 0 and DoesEntityExist(trailerEntity) then
